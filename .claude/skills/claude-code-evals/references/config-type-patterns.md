...
Scenario design guidance for each type of Claude Code configuration. Each section covers what to test, how to structure the scenario, and what makes good assertions for that config type.

For general assertion design, see `assertion-design.md`. For scenario structure, see `scenario-design.md`.
...
**Key challenge**: Distinguishing skill-added behavior from baseline Claude. Claude is already capable — your assertions must target what the skill *specifically* changes.
...
**Assertion strategy**: Test the *method* the skill teaches, not just the outcome. Without the skill, Claude might still solve the problem — but it would use a different approach.
...
**Key challenge**: Many CLAUDE.md instructions codify things Claude already does sometimes. Your assertion must test whether the instruction makes the behavior *consistent* and *resistant to pressure*.
...
**Assertion strategy**: Pair a compliance scenario (does it follow the rule?) with a pressure scenario (does it follow the rule when asked not to?). The pressure test is where CLAUDE.md instructions prove their value.
...
**Assertion strategy**: For hooks, assert on observable side effects (hook output in transcript, behavioral change). For settings, assert that the constraint is respected (agent works within limited tools, uses the configured model).
...
**Key challenge**: The MCP server must be running and accessible during the eval. Eval infrastructure failures (server not started, wrong port) look like assertion failures. Use the `errors` field in results to distinguish infrastructure problems from behavioral ones.
...
**Assertion strategy**: Assert both that the tool was used (process) and that results were incorporated (presence). If the agent could answer the question from training data alone, the assertion may always-pass — test with queries that require current or project-specific information.
...
**Assertion strategy**: Assert on the delegation pattern (Agent tool was used), not just the final output. The value of sub-agent configuration is in the *process*, not just the result.
...
**Key challenge**: Assertions must be calibrated for the weaker model. If you write assertions that only the strongest model passes, you're not comparing — you're just testing one model. Write assertions that reveal *differences*, with some that both models should pass and some that differentiate.
...
**Assertion strategy**: Use identical assertions across model variants so pass rates are directly comparable. Labels (`model: sonnet-4-6`) enable downstream comparison. The delta in pass rates quantifies the model difference.
...
**Assertion strategy**: Include `note:` fields that trace each assertion back to the config component responsible. When an assertion fails, the note tells you which component to investigate. Test components individually first to establish baselines, then test the combo to verify they compose without interference.
