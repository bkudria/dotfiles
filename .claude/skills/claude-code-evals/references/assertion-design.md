...
Write assertions that test what your configuration specifically adds, not what Claude already does.
...
If yes, the assertion tests baseline behavior, not config value. Revise it to target what the configuration specifically adds.
...
|---|---|---|
| **Always-passes** | Tests baseline Claude behavior, not config-added value | Ask: "would Claude do this WITHOUT the config?" If yes, don't assert it |
...
| **Compound** | Tests two things in one assertion (e.g., "uses correct syntax AND explains why") | Split into two separate assertions |
...
Different configuration types add value in different ways. Target assertions accordingly:

| Config Type | Claude's Default | What Config Adds | Good Assertion Targets |
|-------------|-----------------|------------------|----------------------|
...
Always lint assertions before spending money on eval runs:

- **Single rubric**: `pincenez lint rubric.yml` — checks for anti-patterns in one rubric
- **Full eval suite**: `craboodle lint <evals-dir>` — checks all scenarios

Linting catches vague, compound, tautological, always-passes, and unverifiable assertions before they waste LLM calls. Fix flagged issues, then run.

---

## How Assertions Get Graded

Each assertion is graded independently by pincenez (one LLM call per assertion) against the agent's transcript. This independence prevents cross-contamination — earlier verdicts cannot influence later ones.
...
Failures include per-rep evidence explaining why the assertion failed. This evidence helps diagnose whether the issue is in the configuration or the assertion itself.

### Grading notes

The `note:` field on assertions significantly improves grading accuracy. Use it to tell the grader what to look for:
