...
#
# Usage:
#   find-tool-calls.sh <session.jsonl> [--path <pattern>] [--tools <names>]
#
# Options:
...
# With no filters, shows all tool calls (similar to extract-changes.jq but with more detail).
...
if [[ $# -lt 1 ]]; then
  echo "Usage: find-tool-calls.sh <session.jsonl> [--path <pattern>] [--tools <names>]" >&2
  echo "" >&2
  echo "Options:" >&2
...
jq -L "$SCRIPT_DIR" -r '
  ...
  | "\($time)  \(.name | lib::truncate(12))  \(lib::tool_file_path // "-" | lib::truncate(60))\n          \(lib::tool_content_preview(200))"
' "$SESSION_FILE"
