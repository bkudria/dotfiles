...
Reference for the JSON-IN-KDL (JiK) microsyntax. Consult this file when encoding JSON as KDL, decoding JiK back to JSON, or working with `(array)` and `(object)` type annotations in JiK contexts.
