...
```json
{
  ...
  "pending": [
    { "id": "base/lockfile", "required": true, "description": "...", "rendered_prompt": "..." }
  ],
  "disabled_count": 0,
  "suggested_total": 4,
...
**GATE — No prompt extraction. Do NOT write `rendered_prompt` content (or any other field of `collect-required.json`) to bash output, `/tmp/...`, or any side file. Read `$STATE_DIR/collect-required.json` once with the Read tool — that single Read is the canonical pre-dispatch inspection. Then copy each `rendered_prompt` directly into the corresponding Agent tool_use's `prompt` parameter. Do NOT `mkdir`, `for`-loop dump, `jq` enumerate, or echo prompts through Bash.**

For every entry in `pending`, dispatch a `general-purpose` sub-agent with `model: 'haiku'` whose prompt is the entry's `rendered_prompt` from `$STATE_DIR/collect-required.json`, copied verbatim. The runner has already baked in the description, the prompt body, and a directive instructing the agent to write `{"met": true|false, "detail": "<one-line>"}` to its `response_path` using the Write tool. The agent's conversational reply is ignored; only the file matters.
...
**GATE — No prompt extraction (round 2). Same rule as round 1, applied to `collect-suggested.json`.**

For every entry in this round's `pending`, dispatch a `general-purpose` sub-agent with `model: 'haiku'`. After all return, run merge again:
